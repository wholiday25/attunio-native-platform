...
foreach ($aibtemplate in Get-ChildItem -Recurse -Filter '*.json' -File -Exclude 'Win11*', 'X_*', 'aibEntDesktopV2*','*publish*', 'aibentdesktop*', 'Readme.md', 'scripts', '*parameters*', '*.ps1', 'aibRoleDefinition.json') {
...
foreach ($aibtemplate in Get-ChildItem -Recurse -Filter '*.json' -File -Exclude 'Win11*', 'X_*', 'aibEntDesktopV2*','aibentdesktop*', 'Readme.md', 'scripts', '*parameters*', '*.ps1', 'aibRoleDefinition.json', '*.yaml','SingleApp') {
    $imageTemplateName = $aibtemplate.Name -replace ".json", ""
    $imageTemplateFileName = $imageTemplateName + ".json"
    $imageTemplateFileNameParameters = $imageTemplateName + ".parameters" + ".json" 
